...
const ScreenME21N: React.FC<ScreenProps> = ({ tCode, tDescription }) => {
    ...
    const handleSave = () => {
        ...
        setMessage(`Pedido estándar ${po.documentNumber} creado.`);
        setItems([]);
        setHeader({ ...header, vendor: '' });
    };
                    ...
                    <tbody>
                        {items.map((item, idx) => (
                            <tr key={idx}>
                                ...
                                <td><input type="text" className="sap-input w-lg" value={item.shortText} onChange={(e) => handleUpdateItem(idx, 'shortText', e.target.value)} /></td>
                                ...
                                <td><input type="text" className="sap-input w-xs" value={item.plant} onChange={(e) => handleUpdateItem(idx, 'plant', e.target.value)} /></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
